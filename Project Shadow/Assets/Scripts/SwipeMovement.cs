...
public class SwipeMovement : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (!findPos)
        {
            ...
            //this.transform.parent = spawnPos.transform;
            findPos = true;
        }
    }
    ...
    void MovePlayer()
    {
        ...
        if (swipeControl.SwipeUp && Steps.StepLeft>0 && gridScript.grid.gridArray[(int)gridScript.playerGridPosition.x, (int)gridScript.playerGridPosition.y-1].type != 2)
        {
            Steps.StepLeft -= 1;
            nextPos = Vector3.forward;
            canMove = true;
            ...
            gm.turn++;
            gm.playerTurn = true;
            //FindObjectOfType<audioman>().ChooseClip("playerstep");
            GetComponent<AudioSource>().clip = stepSound;
            GetComponent<AudioSource>().Play();

            currentDirection = up;
        }
        if (swipeControl.SwipeDown && Steps.StepLeft > 0 && gridScript.grid.gridArray[(int)gridScript.playerGridPosition.x, (int)gridScript.playerGridPosition.y + 1].type != 2)
        {
            Steps.StepLeft -= 1;
        ...
        if (swipeControl.SwipeLeft && Steps.StepLeft > 0 && gridScript.grid.gridArray[(int)gridScript.playerGridPosition.x - 1 , (int)gridScript.playerGridPosition.y].type != 2)
        {
            Steps.StepLeft -= 1;
        ...
        if (swipeControl.SwipeRight && Steps.StepLeft > 0 && gridScript.grid.gridArray[(int)gridScript.playerGridPosition.x + 1, (int)gridScript.playerGridPosition.y].type != 2)
        {
            Steps.StepLeft -= 1;
